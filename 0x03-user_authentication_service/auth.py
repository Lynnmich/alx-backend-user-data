...
def _hash_password(password: str) -> bytes:
    """Method takes password and return salted hash of the password"""
    return hashpw(password.encode('utf-8'), gensalt())


def _generate_uuid() -> str:
    """Method returns a string representation of a new UUID"""
    return str(uuid4())
...
class Auth:
    ...
    def get_reset_password_token(self, email: str) -> str:
        """Method locates user by email. if user exists, generates a
        UUID then returns token"""
        try:
            user = self._db.find_user_by(email=email)
        except NoResultFound:
            raise ValueError
